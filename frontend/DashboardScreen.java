...
public class DashboardScreen extends JPanel {
    ...
    private JPanel criarConteudo() {
        ...
        header.add(Tema.criarLabel("PAINEL DE CONTROLE" + nomeFaz, Tema.F_TITLE, Tema.GREENL), BorderLayout.WEST);
        JPanel headerR = new JPanel(new FlowLayout(FlowLayout.RIGHT, 6, 0));
        headerR.setBackground(Tema.BG);
        JLabel data = Tema.criarLabel(new SimpleDateFormat("dd/MM/yyyy HH:mm").format(new Date()), Tema.F_SMALL, Tema.TEXT3);
        ...
        scAnimais = new StatCard("ANIMAIS ATIVOS",  String.valueOf(backend.totalAnimais()),       Tema.GREENL, "cadastrados");
        scArea    = new StatCard("FAZENDAS/ÁREAS",  fazendaAreaLabel(),                           Tema.GREEN3, "fazendas/lotes");
        scAlertas = new StatCard("ALERTAS",         String.valueOf(backend.totalAlertas()),       Tema.RED,    "pendentes");
        scColares = new StatCard("COLARES ATIVOS",  String.valueOf(backend.totalColaresAtivos()), Tema.CYAN,   "GPS conectados");
        ...
        topoLista.add(Tema.criarLabel("◈ ANIMAIS MONITORADOS", Tema.F_LABEL, Tema.TEXT3), BorderLayout.WEST);
        ...
        // Timer de auto-refresh a cada 30s
    ...
    public void atualizarDados() {
        scAnimais.atualizar(String.valueOf(backend.totalAnimais()));
        scArea.atualizar(fazendaAreaLabel());
        scAlertas.atualizar(String.valueOf(backend.totalAlertas()));
        scColares.atualizar(String.valueOf(backend.totalColaresAtivos()));
        carregarTabelaAnimais();
    }
    ...
    private String fazendaAreaLabel() {
        ...
        long lotes = backend.fazendaService.listarTodas().stream().mapToLong(f -> f.getLotes().size()).sum();
        return faz + "/" + lotes;
    }
