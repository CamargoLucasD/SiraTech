...
public class FinanceiroScreen extends JPanel {
    ...
    private JPanel criarConteudo() {
        ...
        // ── Abas com ícones SVG (sem emojis) ─────────────────────────────────
        abas.addTab(null, criarAbaResumo());
        JLabel tabResumo = new JLabel("RESUMO FINANCEIRO");
        tabResumo.setIcon(ico("trending-up", 14));
        tabResumo.setIconTextGap(5);
        abas.setTabComponentAt(0, tabResumo);

        abas.addTab(null, criarAbaTransacoes());
        JLabel tabTrans = new JLabel("COMPRA / VENDA");
        tabTrans.setIcon(ico("tag", 14));
        tabTrans.setIconTextGap(5);
        abas.setTabComponentAt(1, tabTrans);
        ...
        titulo.add(new JLabel(ico("trending-up", 20)));
        titulo.add(Tema.criarLabel("GESTÃO FINANCEIRA" + nomeFaz, Tema.F_TITLE, Tema.GREENL));
    ...
    private JPanel criarAbaResumo() {
        ...
        secHeader.add(Tema.criarLabel("RESUMO POR FAZENDA / LOTE", Tema.F_LABEL, Tema.TEXT3));
    ...
    private JPanel criarAbaTransacoes() {
        ...
        formHeader.add(new JLabel(ico("tag", 14)));
        formHeader.add(Tema.criarLabel("REGISTRAR TRANSAÇÃO", Tema.F_LABEL, Tema.TEXT3));
        form.add(formHeader);
        ...
        // ── Botão registrar — padrão Tema.criarBotaoPrimario ─────────────────
        JButton btnReg = Tema.criarBotaoPrimario("REGISTRAR TRANSAÇÃO");
        btnReg.setIcon(ico("save", 14));
        ...
        topoLeft.add(new JLabel(ico("clipboard-list", 14)));
        topoLeft.add(Tema.criarLabel("HISTÓRICO DE TRANSAÇÕES", Tema.F_LABEL, Tema.TEXT3));
        topo.add(topoLeft, BorderLayout.WEST);

        // ── Botão excluir — padrão Tema.criarBotaoPerigo ─────────────────────
        ...
        if (fa != null) {
            ...
            totais.add(Tema.criarLabel(
                    "Saldo: " + (saldo >= 0 ? "+" : "") + "R$ " + String.format("%,.2f", saldo),
    ...
    private void excluirTransacaoSelecionada() {
        ...
        if (row < 0) {
            JOptionPane.showMessageDialog(this, "Selecione uma transação!", "Aviso", JOptionPane.WARNING_MESSAGE);
            return;
        }
        Fazenda fa = backend.getFazendaAtiva();
        if (fa == null) return;
